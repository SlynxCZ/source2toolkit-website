...
* **Sounds**: Sound events sent to exactly the players you pick, stopped or changed by guid, per-player volume channels, and a hook on every sound the game sends.
* **Transmit**: Per-viewer entity visibility -- hide players and entities from some clients and not others, owner-following, and a CheckTransmit hook.
...
* **[Crash Handler](/docs/crash-handler)**: Minidumps processed on the server, plugin frames symbolized to `function @ file:line`, a suspected culprit and a Discord report.
