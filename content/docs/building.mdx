...
- KHook headers — Metamod:Source's own detour library, taken from your Metamod checkout
...
<Callout type="info">
  The SDK bundles nothing but its own sources. **Metamod:Source** (for KHook),
...
```cmake
resolve_dep(METAMOD   MMSOURCE_DEV)
resolve_dep(SOURCESDK HL2SDKCS2)
resolve_dep(PROTOBUFS CSGO_PROTO)
```

All three are provided via environment variables:

| Variable | Description |
|---|---|
| `MMSOURCE_DEV` | Path to your Metamod:Source checkout — KHook lives in its `third_party/khook` |
...
# Windows (PowerShell)
$env:MMSOURCE_DEV = "C:\path\to\metamod-source"
